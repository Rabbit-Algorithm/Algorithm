package BFS_바킹독_과제._2주차.벽부수고이동하기2_14442;
...
public class Main_진은수 {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (nx >= 0 && nx < xSize && ny >= 0 && ny < ySize) {
                    ...
                    if (map[ny][nx] == 0 && !visited[ny][nx][now.brokenCount] && now.brokenCount >= 0 && now.brokenCount <= brokenNum) {
                        ...
                        queue.add(new Node(nx, ny, now.depth + 1, now.brokenCount));
                    }
                    if (map[ny][nx] == 1 && !visited[ny][nx][now.brokenCount] && now.brokenCount >= 0 && now.brokenCount < brokenNum) {
                        visited[ny][nx][now.brokenCount] = true;
                        queue.add(new Node(nx, ny, now.depth + 1, now.brokenCount+1));
                    }
    ...
    private static class Node {
        ...
        public Node(int x, int y, int depth, int brokenCount) {
            this.x = x;
            this.y = y;
            this.depth = depth;
